...
from .ast_nodes import (
    ...
    UnaryExprNode, CallExprNode, AssignmentExprNode, ASTVisitor
)
...
class Parser:
    ...
    def is_var_declaration(self) -> bool:
        ...
        if self.check(TokenType.KW_INT, TokenType.KW_FLOAT, TokenType.KW_BOOL, TokenType.IDENTIFIER):
            ...
            if self.check(TokenType.IDENTIFIER):
                self.advance()
                if self.check(TokenType.ASSIGN, TokenType.SEMICOLON):
                    self.current = saved
                    return True
    ...
    def parse_relational(self) -> ExpressionNode:
        ...
        while self.match(TokenType.LT, TokenType.LTE,
    ...
    def finish_call(self, callee: ExpressionNode) -> CallExprNode:
        ...
        return CallExprNode(
            callee=callee if isinstance(callee, IdentifierExprNode)
            else IdentifierExprNode(name="unknown", line=callee.line, column=callee.column),
            arguments=arguments,
            line=callee.line,
    ...
    def parse_primary(self) -> ExpressionNode:
        ...
        if self.match(TokenType.IDENTIFIER):
            token = self.previous()
            return IdentifierExprNode(name=token.lexeme,
                                      line=token.line, column=token.column)
